...
async def get_orders():
    try:
        query = "SELECT * FROM orders"  # Adjust based on your actual schema
        results = await database.fetch_all(query=query)

        # Transform results to a list of orders
        orders = []
        for row in results:
            order = {
                "id": row["id"],
                ...
                "items": row["items"],  # Ensure this is properly parsed if it's a JSON string
            }
            orders.append(order)
