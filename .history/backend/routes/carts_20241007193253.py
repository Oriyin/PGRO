...
async def checkout(order: Order):
    ...
    # Get the current time in UTC and convert to Thailand time (UTC+7)
    now_utc = datetime.utcnow()
    thailand_time = now_utc + timedelta(hours=7)
    ...
    order_id = await database.execute(query, values={
        ...
        "created_at": thailand_time  # Save the time in Thailand timezone
    })
