...
async def create_user(user: UserCreate):
    ...
    # Insert the user into the database with the current Thai time
    result = await insert_user(user.username, user.password_hash, user.email, current_time)
...
async def update_last_login(user_id: int):
    tz = pytz.timezone('Asia/Bangkok')  # Set timezone to Asia/Bangkok
    current_time = datetime.now(tz)  # Get the current time in Asia/Bangkok timezone
