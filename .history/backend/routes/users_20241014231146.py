...
# Modify the insert_user function to store the created_at timestamp
async def insert_user(username: str, password_hash: str, email: str, created_at: datetime):
    query = """
...
# Endpoint สำหรับการเพิ่มความคิดเห็น
@router.post("/feedback/")
async def submit_feedback(feedback: FeedbackCreate, username: str):
    current_user = await get_current_user(username)
    result = await insert_feedback(user_id=current_user['user_id'], feedback_text=feedback.feedback_text)
    if not result:
...
# Endpoint สำหรับดึงความคิดเห็นของผู้ใช้ที่ล็อกอิน
@router.get("/feedback/", response_model=List[dict])
async def get_user_feedback(username: str):
    current_user = await get_current_user(username)
    feedbacks = await get_feedback_by_user(user_id=current_user['user_id'])
    return feedbacks

@router.post("/feedback/")
async def submit_feedback(feedback: FeedbackCreate, user_id: int):
    result = await insert_feedback(user_id=user_id, feedback_text=feedback.feedback_text)
    if not result:
        raise HTTPException(status_code=400, detail="Unable to submit feedback")
    return {"message": "Feedback submitted successfully", "feedback": result}

# Endpoint สำหรับดึงความคิดเห็นทั้งหมด
@router.get("/feedback/", response_model=List[dict])
async def get_all_feedbacks():
    feedbacks = await get_all_feedback()
    return feedbacks

# Endpoint สำหรับการนับ Feedback ทั้งหมด
@router.get("/feedback/count")
async def get_feedback_count():
    count = await count_all_feedback()
    return {"total_feedback": count}

# Endpoint สำหรับการนับ Feedback ต่อผู้ใช้
@router.get("/feedback/user-count/{user_id}")
async def get_feedback_count_by_user(user_id: int):
    count = await count_feedback_by_user(user_id)
    return {"user_feedback_count": count}