...
async def get_recent_orders(limit: int = 5):
    ...
    SELECT 
        ...
        o.total_amount, 
        u.username, 
        json_agg(json_build_object('quantity', oi.quantity, 'product_id', p.product_id, 'product_name', p.product_name)) AS products
    FROM orders o
    ...
    """
    try:
        # Fetch recent orders along with product details
