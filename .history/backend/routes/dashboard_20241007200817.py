...
async def get_latest_orders(limit: int = 10):
    """
    Get the latest orders with item details (name, quantity, price), limited to showing only 3 items per order.
    ...
    for row in results:
        items = row["items"][:3]  # Limit to the first 3 items
        detailed_items = []
        ...
        for item in items:
            ...
            # Query to get the product name and price
        ...
        orders.append({
            ...
            "items": detailed_items  # Only the first 3 items
        })
