...
async def update_last_login(user_id: int):
    # อัปเดต last_login ด้วยเขตเวลาประเทศไทย
    now_in_thailand = datetime.now(THAILAND_TZ)
    query = """
    UPDATE users
    SET last_login = :last_login
    WHERE user_id = :user_id
    """
    values = {"last_login": now_in_thailand, "user_id": user_id}
    await database.execute(query=query, values=values)

async def get_recent_logins_from_db(limit: int):
    query = f"""
