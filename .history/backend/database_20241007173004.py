...
# ฟังก์ชันยกเลิกการเชื่อมต่อฐานข้อมูล
async def disconnect_db():
    await database.disconnect()
    print("Database disconnected")

# ฟังก์ชันสำหรับอัปเดตเวลาล็อกอินล่าสุด
async def update_last_login(user_id: int):
    current_time = datetime.now(THAI_TZ)  # ใช้เขตเวลาไทย
    query = """
    UPDATE users
    SET last_login = :last_login
    WHERE user_id = :user_id
    """
    values = {"last_login": current_time, "user_id": user_id}
...
# ฟังก์ชันตรวจสอบ email ซ้ำ
async def get_user_by_email_only(email: str):
    query = "SELECT * FROM users WHERE email = :email"
    return await database.fetch_one(query=query, values={"email": email})

# ฟังก์ชันดึงผู้ใช้ตาม username
async def get_user(username: str):
    query = "SELECT * FROM users WHERE username = :username"
    return await database.fetch_one(query=query, values={"username": username})

# ฟังก์ชันดึงผู้ใช้ตาม email และ password
async def get_user_by_email_and_password(email: str, password_hash: str):
