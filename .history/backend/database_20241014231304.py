...
# ฟังก์ชันสำหรับการเพิ่ม feedback
async def insert_feedback(user_id: int, feedback_text: str):
    tz = pytz.timezone('Asia/Bangkok')  # กำหนดเขตเวลาเป็น Asia/Bangkok
    current_time = datetime.now(tz)  # บันทึกเวลาปัจจุบันในเขตเวลาไทย
    
    query = """
    INSERT INTO feedback (user_id, feedback_text, created_at)
    VALUES (:user_id, :feedback_text, :created_at)
    RETURNING id, user_id, feedback_text, created_at
    """
    values = {"user_id": user_id, "feedback_text": feedback_text, "created_at": current_time}
    return await database.fetch_one(query=query, values=values)

# ฟังก์ชันสำหรับการดึง feedback ของผู้ใช้
async def get_feedback_by_user(user_id: int):
    ...
    feedbacks = [
        {
            ...
            "created_at": row["created_at"].astimezone(tz)  # แปลงเวลาเป็นเขตเวลาไทย
        }
        for row in rows
    ]
    return feedbacks

# ฟังก์ชันสำหรับการดึง feedback ทั้งหมด
...
# ฟังก์ชันสำหรับการนับจำนวน feedback ทั้งหมด
async def count_all_feedback():
    query = "SELECT COUNT(*) FROM feedback"
    result = await database.fetch_one(query=query)
    return result['count']

# ฟังก์ชันสำหรับการนับจำนวน feedback ต่อผู้ใช้
async def count_feedback_by_user(user_id: int):
    query = "SELECT COUNT(*) FROM feedback WHERE user_id = :user_id"
