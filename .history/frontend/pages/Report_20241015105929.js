...
const Report = () => {
  ...
  const [loadingSales, setLoadingSales] = useState(true); // Separate loading states
  ...
  useEffect(() => {
    const fetchSalesData = async () => {
      try {
        ...
        setLoadingSales(false); // Set loading to false after sales data is fetched
      } catch (error) {
    ...
    const fetchWeeklyReport = async () => {
      try {
        ...
        setLoadingWeekly(false); // Set loading to false after weekly report is fetched
      } catch (error) {
  ...
  // Show loading spinner until both sales data and weekly report are loaded
  if (loadingSales || loadingWeekly) {
    return (
    ...
    <>
      ...
      <Box sx={{ padding: '90px' }}>
        ...
        <Grid container spacing={3}>
          ...
          {/* Gauge Chart: Weekly Performance */}
          <Grid item xs={12} lg={4}>
            <Paper elevation={3} sx={{ padding: '20px', backgroundColor: colors.lightPurple }}>
              <Typography variant="h6" gutterBottom color={colors.purple}>
                Weekly Performance
              ...
              <Gauge
                ...
                max={100000}  // Adjust this max value based on your actual total sales range
                startAngle={0}
                endAngle={360}
                innerRadius="80%"
                outerRadius="100%"
