...
const ReportPage = () => {
  ...
  const [newRegistrationsData, setNewRegistrationsData] = useState([]); // New state for new registrations
  ...
  useEffect(() => {
    ...
    const fetchNewRegistrationsData = async () => {
      const response = await fetch('/api/users/new-registration'); // Fetch new registrations data
      const data = await response.json();
      setNewRegistrationsData(data); // Set new registrations data
    };

    const fetchData = async () => {
      await fetchSalesData();
      await fetchQuarterlySalesData();
      await fetchNewRegistrationsData(); // Call the new fetch function
      setLoading(false);
    };
  ...
  return (
    <Box sx={{ padding: '20px' }}>
      ...
      {/* New Registrations */}
      <Paper elevation={3} sx={{ padding: '20px', marginTop: '20px', backgroundColor: colors.lightBlue }}>
        <Typography variant="h6" gutterBottom color={colors.purple}>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={newRegistrationsData}>
            ...
            <YAxis />
            <Tooltip />
            <Bar dataKey="total_users" fill={colors.purple} />
          </BarChart>
        </ResponsiveContainer>
      </Paper>
    </Box>
